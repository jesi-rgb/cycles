...
{:else}
    <button
        ...
        class="relative btn btn-circle btn-lg btn-accent dark:btn-accent text-4xl flex flex-row rounded-full cursor-pointer fraction"
    >
        <span
            class:text-3xl={ccountLength + tcountLength >= 4}
            class="font-[1000]"
        >
            <sup>{currentCount.toString()}</sup>⁄<sub>
                {habit.target_count.toString()}
